...
exports.HardhatNetwork = void 0;
const ethers_1 = require("ethers");
const EvmContractManager_1 = require("./EvmContractManager");
...
class HardhatNetwork {
    ...
    contracts;
    contractSigner;
    constructor(startedHardhatContainer, contractDeployerAddress) {
        this.startedHardhatContainer = startedHardhatContainer;
        this.contractDeployerAddress = contractDeployerAddress;
        this.ethersRpcProvider = new ethers_1.ethers.providers.StaticJsonRpcProvider(startedHardhatContainer.rpcUrl);
        this.contractSigner = this.ethersRpcProvider.getSigner(contractDeployerAddress);
        this.contracts = new EvmContractManager_1.EvmContractManager(startedHardhatContainer, this.contractSigner, this.ethersRpcProvider);
    }
    /**
    ...
    async createTestWallet() {
        const testWalletAddress = ethers_1.ethers.Wallet.createRandom().address;
        await this.activateAccount(testWalletAddress);
        await this.fundAddress(testWalletAddress, ethers_1.ethers.constants.MaxInt256);
        return {
            testWalletAddress,
    ...
    async fundAddress(address, amountToFund) {
        // convert amount to fund to hex
        const amountToFundInHex = ethers_1.ethers.BigNumber.from(amountToFund).toHexString();
    ...
    async generate(numBlocks) {
        if (ethers_1.ethers.BigNumber.from(numBlocks).lte(ethers_1.ethers.constants.Zero)) {
            throw Error('Minimum of 1 block needs to be generated.');
        }
        await this.startedHardhatContainer.call('hardhat_mine', [(0, Utils_1.toZeroStrippedHex)(numBlocks)]);
    }
    /**
        ...
        return this.startedHardhatContainer.call('eth_sendTransaction', [
            {
                from,
                to,
                value: (0, Utils_1.toZeroStrippedHex)(value),
            },
        ]);
    }
}
exports.HardhatNetwork = HardhatNetwork;
